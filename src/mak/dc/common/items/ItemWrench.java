                    ...
                    case 0:
                        if (world.getBlock(x, y, z) instanceof BlockDeadCraft && te.isUserAllowed(username)) {
                            if (te instanceof IPowerReceiver) {
                                ((IPowerReceiver) te).updatePowerSource(getBlockCoord(stack));
                                removeBlockCoord(stack);
                            } else if (te instanceof IPowerSender) {
                                saveBlockCoord(stack, x, y, z);
                            }
                            if (player.isSneaking()) {
                                ((BlockDeadCraft) world.getBlock(x, y, z)).onWrenched(world, x, y, z, player, side, hitX, hitY, hitZ);
                            }
                        }
                        break;
                    case 1:
