...
public class ItemCrystal extends Item {
	...
	public static final int[] MAXCHARGE = { ConfigLib.CRYS_DEFMAXCHARGE,
			4 * ConfigLib.CRYS_DEFMAXCHARGE, 16 * ConfigLib.CRYS_DEFMAXCHARGE };
	...
	public static int chargeItem(ItemStack stack, int chargeAmount) {
		...
		if (curentCharge >= MAXCHARGE[stack.getItemDamage()])
			return chargeAmount;
		if (curentCharge + chargeAmount > MAXCHARGE[stack.getItemDamage()]) {
			tag.setInteger("charge", MAXCHARGE[stack.getItemDamage()]);
			stack.setTagCompound(tag);
			return curentCharge + chargeAmount
					- MAXCHARGE[stack.getItemDamage()];
		} else {
	...
	public static boolean canDischarge(ItemStack crys, int amount) {
		if (!crys.hasTagCompound())
			return false;
		NBTTagCompound tag = crys.getTagCompound();
		if (tag.getBoolean("creativeSpawn")) return true;
		int curentCharge = tag.getInteger("charge");
		if (curentCharge - amount < 0) return false;
		else return true;
	}
	...
	public static boolean isCreative(ItemStack is) {
		if (!is.hasTagCompound())
			return false;
		return is.getTagCompound().hasKey("creativeSpawn")
				&& is.getTagCompound().getBoolean("creativeSpawn");
	}
	...
	public void addInformation(ItemStack is, EntityPlayer player, List l,
		...
		if (is.hasTagCompound() && is.getTagCompound().hasKey("charge")) {
			if (is.getTagCompound().getBoolean("creativeSpawn")) {
				l.add(StatCollector.translateToLocal("dc.charge") + " : "
						+ EnumChatFormatting.YELLOW
						+ StatCollector.translateToLocal("dc.infinite")
						+ EnumChatFormatting.RESET);
			} else {
				l.add(StatCollector.translateToLocal("dc.charge") + " : "
						+ EnumChatFormatting.YELLOW
						+ is.getTagCompound().getInteger("charge") + "/"
						+ MAXCHARGE[is.getItemDamage()]
						+ EnumChatFormatting.RESET);
			}
			if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
				l.add(StatCollector.translateToLocal("dc.crystal.info.tier")
						+ " : " + EnumChatFormatting.DARK_GREEN
						+ (is.getItemDamage() + 1));
				if (is.getTagCompound().getBoolean("creativeSpawn"))
					l.add(StatCollector.translateToLocal("dc.info.isCreative"));
			} else {
				l.add(EnumChatFormatting.YELLOW
						+ StatCollector.translateToLocal("dc.info.holdShift"));
			}
		}
	}
	...
	public double getDurabilityForDisplay(ItemStack stack) {
		...
		if (tag == null)
			return 0;
		return 1d - (double) tag.getInteger("charge")
				/ MAXCHARGE[stack.getItemDamage()];
	}
	...
	public void registerIcons(IIconRegister registerIcon) {
		for (int i = 0; i < tiers.length; i++) {
			icons[i] = registerIcon.registerIcon(Textures.CRYSTAL_TEXT_LOC
					+ ".tier" + i);
		}
	}
	...
	public boolean showDurabilityBar(ItemStack stack) {
		return stack.hasTagCompound()
				&& !stack.getTagCompound().getBoolean("creativeSpawn");
	}
