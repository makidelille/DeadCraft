...
public class ContainerDeadCraft extends Container {
    ...
    public boolean canInteractWith(EntityPlayer entityplayer) {
        if (!te.isUserAllowed(entityplayer.getCommandSenderName())) {
            entityplayer.addChatComponentMessage(new ChatComponentText("the block is locked"));
        }
        return te == null ? true : te.isUserAllowed(entityplayer.getCommandSenderName()) && entityplayer.getDistanceSq(te.xCoord, te.yCoord, te.zCoord) <= 64;
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int i) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            re = stack.copy();
            if (i >= 27) {
                if (!mergeItemStack(stack, 0, 27, false)) return null;
            } else if (i < 27) {
                if (!mergeItemStack(stack, 27, 36, false)) return null;
            }
